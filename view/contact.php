...
<div class="card container-sm col-lg-8 col-xl-6 mt-5 mb-5 p-5 text-center">
    ...
    <?= (!empty($reception)) ? '<div class="alert alert-danger text-center">' . $reception . '</div>' : null; ?>

    <!--Formulaire de contact-->
    <div class="container mt-5 col-lg-10 col-xl-9">
        <form method="post">
            <div class="form-group input-group-lg">
                <input class="form-control" value="<?= (!empty($title)) ? $title : null; ?>" name="title_forms"
                       type="text"
                       placeholder="<?= $content[0] ?>" maxlength="30"
                       pattern="[A-Za-z0-9 '-]+$" required>
            </div>
            <div class="form-group input-group-lg">
            <textarea class="form-control" name="text_forms" placeholder="<?= $content[1] ?>" cols="20" rows="5"
                      maxlength="255"
